...
if($cfg['source']=='mssql' && !function_exists('mssql_connect')){
    exit("[Error] mssql_connect() not exists, mssql extension required.\n");
}
if( $cfg['source']=='sqlite3' && !class_exists('SQLite3')) {
    exit("[Error] mssql_connect() not exists, mssql extension required.\n");
}
if( $cfg['source']=='sqlite2' && !function_exists('sqlite_open')) {
    exit("[Error] mssql_connect() not exists, mssql extension required.\n");
}
...
if($cfg['source']=='mssql'){
    ...
    $conn=mssql_connect($cfg['mssql']['host'],$cfg['mssql']['user'],$cfg['mssql']['passwd']) or exit(mssql_get_last_message());
    mssql_select_db($cfg['mssql']['db'],$conn) or exit(mssql_get_last_message());
}


# connect sqlite 连接sqlite, 2 or 3
if($cfg['source']=='sqlite3'){
    if($cfg['mssql']['port']!=''){
        $cfg['mssql']['host']=$cfg['mssql']['host'] . (PHP_OS=='WINNT' ? ',' : ':') .$cfg['mssql']['port'];
    }
    $conn=mssql_connect($cfg['mssql']['host'],$cfg['mssql']['user'],$cfg['mssql']['passwd']) or exit(mssql_get_last_message());
